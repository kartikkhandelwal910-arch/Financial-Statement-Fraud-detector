...
# Consistent palette across every chart in the dashboard
COLOR_FLAGGED = "#c0392b"
COLOR_CLEAN = "#2e7d32"
COLOR_THRESHOLD = "#7f7f7f"
COLOR_KNOWN_CASE = "#f39c12"
FIGSIZE_WIDE = (10, 5.5)
...
def plot_comparison_bar(mscore_df: pd.DataFrame, ax=None):
    ...
    bars = ax.barh(latest["ticker"], latest["MScore"], color=colors, edgecolor="white", height=0.6)
    ...
    ax.set_title("M-Score by Company — Cross-Sectional Comparison")
...
def plot_mscore_trend(mscore_df: pd.DataFrame, tickers=None, ax=None):
    """
    NEW: Multi-year M-Score trend line, one line per ticker, instead of
    only the latest-year snapshot. This is the single upgrade the original
    README flagged first ("Multi-year M-Score trend line per company
    instead of single latest-year score") and it matters because a company
    can cross the threshold gradually — the trend shows *when* it started
    drifting, not just where it landed.
    ...
    ax.set_title("M-Score Trend Over Time")
...
def plot_dashboard(mscore_df: pd.DataFrame, top_ticker: str = None):
    """
    NEW: One-call combined dashboard figure — trend line (top), comparison
    bar (bottom-left), radar for the top-flagged company (bottom-right).
    This is the "polish" upgrade: instead of three separate plt.show()
    calls, one figure you can screenshot or save straight into a resume
    portfolio / README.
    """
    ...
    fig.suptitle("Beneish M-Score — Forensic Earnings Screen", fontsize=14, fontweight="bold")
